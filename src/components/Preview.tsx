...
const Preview: React.FC<PreviewProps> = ({ project, onExitPreview }) => {
  ...
  useEffect(() => {
    import('./Engine.js').then((module) => {
      ...
      if (canvas) {
        gameEngineRef.current = new Engine(project, {}, {
          canvasId: canvasId,
        ...
        gameEngineRef.current.setCurrentScene = function(scene: Scene | null) {
    ...
    <div className="preview">
      <div className="preview-header">
        <div className="preview-title">
          <h1>{project.name}</h1>
          {currentSceneName && <span className="preview-scene-name">/ {currentSceneName}</span>}
        </div>
        <button className="btn btn-secondary" onClick={onExitPreview}>
          <ArrowLeft size={16} /> Exit Preview
        </button>
      ...
      <div className="preview-content">
        <div id={canvasId} className="pace-canvas">
        </div>
      </div>
    </div>
  );
